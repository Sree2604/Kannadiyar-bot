...
function ProductCards({ product }) {
      ...
      const fetchWishlistItems = async () => {
        try {
          ...
          if (Array.isArray(data)) {
            setWishlistItems(data.map((item) => item.id)); // Set the wishlist items for the logged-in customer
          } else {
            setWishlistItems([]);
          }
        } catch (error) {
  ...
  return (
    <>
      <ToastContainer />
      <div className="sm:flex sm:flex-col sm: mt-3 ml-12 font-content lg:grid lg:grid-cols-4 lg:gap-2 ">
        {!product ? (
          ...
          <>
            {randomProducts.map((val) => (
              ...
              >
                ...
                <Card.Text className="sm: ml-1 pl-2">
                  {val.sub_category}
                </Card.Text>
                <Card.Body>
                  <div className="flex">
